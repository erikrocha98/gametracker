...
def test_remove_rating_success():
    repo = FakeUserGameRepository()
    repo.internal_ids["rawg-1"] = 10
    entry = repo.add(user_id=1, game_id=10)
    entry.rating = 4.0

    uc = RemoveRatingUseCase(repository=repo)
    uc.execute(user_id=1, external_id="rawg-1")

    assert repo.get(user_id=1, game_id=10).rating is None


def test_remove_rating_unknown_game_raises():
    repo = FakeUserGameRepository()
...
def test_remove_rating_game_not_in_collection_raises():
    repo = FakeUserGameRepository()
    repo.internal_ids["rawg-1"] = 10
